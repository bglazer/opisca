...
def checkParams(_params) {
  _params.arguments.collect{
    if (it.value == "viash_no_value") {
      println("[ERROR] option --${it.name} not specified in component method")
      println("exiting now...")
        exit 1
    }
  }
...
process method_process {
  ...
  script:
    ...
    if (params.test)
      ...
      # Adding NXF's `$moduleDir` to the path in order to resolve our own wrappers
      export PATH="./:${moduleDir}:\$PATH"
      ./${params.method.tests.testScript} | tee $output
      """
    else
      """
      # Some useful stuff
...
workflow method {
  ...
  result_ = method_process(id_input_output_function_cli_params_)
...
workflow test {
  ...
  main:
  params.test = true
  params.method.output = "method.log"

  Channel.from(rootDir) \
    | filter { params.method.tests.isDefined } \
    | map{ p -> new Tuple3(
        "tests",
        params.method.tests.testResources.collect{ file( p + it ) },
        params
    )} \
    | method
